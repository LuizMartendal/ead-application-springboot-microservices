...
public class CourseController {
    ...
    public ResponseEntity<Object> deleteCourse(@PathVariable UUID courseId) {
        ...
        log.debug("DELETE deleteCourse courseId {} deleted successfully", courseId);
    ...
    public ResponseEntity<Object> updateCourse(@PathVariable UUID courseId, @RequestBody @Valid CourseDto courseDto) {
        log.debug("PUT updateCourse courseId {} and courseDto {} received", courseId, courseDto.toString());
        Optional<CourseModel> courseModelOptional = courseService.findById(courseId);
        if (courseModelOptional.isEmpty()) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Course not found.");
        }
        ...
        courseModel.setLastUpdateDate(LocalDateTime.now(ZoneId.of("UTC")));
        courseService.save(courseModel);
        log.debug("PUT updateCourse courseModel updated {} ", courseModel.toString());
        log.info("Course updated successfully courseModel {} ", courseModel.toString());
